function contractTriggers=scanDailyRDiff(base,base_profiling,contracts,date0,method,doalldates,genfigsdaily)
...
for ci=1:nc
    try
        ...
        voldiff=ttd{1}.Volume;
        
        if (method==1)
            [thresh,entry,nextDayEntryCheck,priorientry]=strategy_swing_xdiff_comprehensinve(ttd{1},voldiff);
        end
        if (method==2)
            [thresh,entry,nextDayEntryCheck,priorientry]=strategy_swing_xdiff_explosivevol(ttd{1},voldiff);
        end
        if (method==3)
            [thresh,entry,nextDayEntryCheck,priorientry]=strategy_swing_xdiff_aggresivevol(ttd{1},voldiff);
        end
        if (method==4)
            [thresh,entry,nextDayEntryCheck,priorientry]=strategy_swing_xdiff_xaggresivevol(ttd{1},voldiff);
        end
        

        entryForTomorrow= entry & priorientry==0;
        enteredTomorrow= entryForTomorrow & nextDayEntryCheck;  
        datesForTomorrow=shiftpad(ttd{1}.Date,-1);

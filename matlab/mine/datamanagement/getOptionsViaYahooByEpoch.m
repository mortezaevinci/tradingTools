function [data,jdata,requestObj,crumb] = getOptionsViaYahooByEpoch(symbol,epoch,requestObjIn,crumbin)
    ...
    %% Construct an URL to obtain the crumb value that is linked to the session cookie. 
     ...
     uri = matlab.net.URI(['https://query2.finance.yahoo.com/v7/finance/options/', upper(symbol)],'date',epoch);
...
%https://query1.finance.yahoo.com/v7/finance/chart/AAPL?range=1d&interval=1m&indicators=quote&includeTimestamps=true&crumb=I.2cJh8rh.E
    ...
    if (isempty(crumb))
         uri = matlab.net.URI(['https://query2.finance.yahoo.com/v7/finance/options/', upper(symbol) ],'date',epoch,'literal');
    else
    uri = matlab.net.URI(['https://query2.finance.yahoo.com/v7/finance/options/', upper(symbol) ],'date',epoch,...
        'crumb',    crumb,...
        'literal');

...
try
    ...
    [dt,~] = tzoffset(t1);
    hh=-hours(dt);
     startdate = posixtime(dt1+hours(hh));
     enddate = posixtime(dt2+hours(hh));
     ...
     doprepost='true';
%      if (strcmp(interval,'1m')==1 || strcmp(interval,'2m')==1 || strcmp(interval,'10m')==1 || strcmp(interval,'15m')==1|| strcmp(interval,'30m')==1|| strcmp(interval,'1h')==1|| strcmp(interval,'4h')==1)
%          doprepost='true';
%      end
     
     tr=timerange(dt1,dt2);
    disp(['yahoo api ',datestr(dt1), ' ~',datestr(dt2),' ',interval]);
...
%https://query1.finance.yahoo.com/v7/finance/chart/AAPL?range=1d&interval=1m&indicators=quote&includeTimestamps=true&crumb=I.2cJh8rh.E
    ...
    if(strcmp(response, 'NotFound'))
        ...
        if (isempty(data))
            disp([failed symbol ' at ' date1,' to ',date2,' ' ,num2str(interval)])
        else
            data(data.Date>dt2,:)=[]; 
        end
       %disp('Received data');
...
try
   ...
   gmtoffsethours=jdcr.meta.gmtoffset/3600;
    dt=datetime(jdcr.timestamp, 'ConvertFrom', 'posixtime','TimeZone','America/New_York');%+hours(gmtoffsethours);
    
    timestamp_regularhour=jdcr.timestamp(1);
    try
   timestamp_regularhour=jdcr.meta.tradingPeriods.regular(end).start;
 catch
 
    end
    
    procData=table(dt,jdcq.open,jdcq.high,jdcq.low,jdcq.close,jdcq.close,jdcq.volume,jdcr.timestamp-timestamp_regularhour,'VariableNames',{'Date','Open','High','Low','Close','AdjClose','Volume','Seconds'});

function [thresh,entry,nextDayEntryCheck,priorientry]=strategy_swing_xdiff_explosivevol(ttd,voldiff)
maxLength=5;
smaLength=10;
outlierMultiplier=1;
outlierMultiplierMax=.25;
...
thresh.stpLmtDiffPerc=linearMap(ttd.Close,0,30,1.07,1.02);
...
thresh.AbsVol=max(20000,thresh.Vol/100);
thresh.AbsAtrPerc=.25; %percentile

thresh.VolRate=max(thresh.Vol/20,10000);
...
minorsmareversal= ((ttd.Close >= sma6 & ttd.Close <sma21) | (abs(sma21-sma6)<thresh.matr & shiftpad(sma6,1)<shiftpad(sma21,1)));
...
% happen
happenedVolEntry=shift(voldiff,-1)>min(thresh.DiffVolAve,thresh.DiffVolMax);
priorientry=movmax(shiftpad(happenedVolEntry,1),[5,0]);

try
    
fprintf("Conds:\t%s e(%d)\t Priori:\t%s p(%d) o(%d)", ...
    num2str(entryConditions(end,:)), entry(end),...
    num2str(happenedVolEntry(end-5:end)'),priorientry(end)==0,...

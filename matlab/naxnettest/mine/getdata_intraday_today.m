...
[t,j,rq,c]=getMarketDataViaYahooChart('AAPL', '1d', '1m');

date='today';

for i=1:numel(symbols)
    symbols{i}
   [tm,jm,~,~]=getMarketDataViaYahooByPeriod(symbols{i}, date,'tomorrow', '1m',rq,c); 
   name=['Z:\My files\Project trading\traderdata\data\' filefriendlysymbol(symbols{i}) ' minute ' datestr(datetime(),"yyyy-mm-dd") '.mat'];
   save(name,'tm','jm');
end
...
[~,jmo,rq,c]=getMarketDataViaYahooChart('AAPL', '1y', '1mo');

for i=1:numel(symbols)
    try
    symbols{i}
   [ty,jy,~,~]=getMarketDataViaYahooChart(symbols{i}, 'max', '3mo',rq,c); 
   name=['Z:\My files\Project trading\traderdata\data\' filefriendlysymbol(symbols{i}) ' quarterly ' datestr(datetime(),"yyyy-mm-dd") '.mat'];
   save(name,'ty','jy');
    catch
       disp('failed to get yearly data'); 
    end
end 
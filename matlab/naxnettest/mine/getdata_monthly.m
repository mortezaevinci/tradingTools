...
[~,~,rq,c]=getMarketDataViaYahooChart('AAPL', '1y', '1mo');

for i=1:numel(symbols)
    symbols{i}
   [tmo,jmo,~,~]=getMarketDataViaYahooChart(symbols{i}, '1y', '1mo',rq,c); 
   name=['Z:\My files\Project trading\traderdata\data\' filefriendlysymbol(symbols{i}) ' monthly ' datestr(datetime(),"yyyy-mm-dd") '.mat'];
   save(name,'tmo','jmo');
end
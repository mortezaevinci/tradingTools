function layout=graphStrategy_full_t7_init(panel,looperParams,params,result)
layout=[];
try
...
%% entries
plotcount=1;
layout.pmain{plotcount}=plot(datetime(),0,'m^','linewidth',2,'ButtonDownFcn',@generalPlotCallback);
plotcount=plotcount+1;
layout.pmain{plotcount}=plot(datetime(),0,'mv','linewidth',2,'ButtonDownFcn',@generalPlotCallback);
plotcount=plotcount+1;
...
if (~isempty(indicatorindex))
    ...
    layout.lowerx{axi}=plot(datetime(),0);
    legend(params.lower_indicators{axi});%calculations.indicators{1}.lower.Properties.VariableNames(indicatorindex));
else

...
threadPlot_single.afterEach(@(x) graphStrategy_full_t5(panel{x{1}},x{2},x{3},x{4},1)); 
threadPlot = parallel.pool.DataQueue;
threadPlot.afterEach(@(x) graphStrategy_t5(panel{x{1}},x{2},x{3},x{4}));
...
%play(Audio); 
 ...
 %;disp('graph');
if ((mainticks{si}.params.graph==2 || (mainticks{si}.params.graph==1 && somethingtodo)) && KEY_IS_PRESSED(1)~='z')

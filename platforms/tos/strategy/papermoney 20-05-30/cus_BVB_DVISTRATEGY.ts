...
input thresh_lowRS = -1;#-0.25;
input thresh_highRS = 5;#1;
input thresh_drsi_100 = 50;# 25;
input thresh_drs = 0;
input thresh_ddvi = -0.75;
...
if (dfindend3 > 0 or doOnlyRealtime == 0)
{
 ...
 #   and rs > thresh_lowRS and rs < thresh_highRS 
  ...
  #  and dsmax / close * 100 > thresh_sma);
    ...
    dncond = breakoutonly or dncondb==575;
...
AddOrder(OrderType.SELL_TO_CLOSE, EntryPrice() <> open and pbuyd  and marketTime, open[-1], tradesize, Color.GREEN, Color.GREEN);
AddOrder(OrderType.BUY_TO_CLOSE, EntryPrice() <> open and pselld and marketTime, open[-1], tradesize, Color.RED, Color.RED);

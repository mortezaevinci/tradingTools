...
then {
    ...
    PP = roundup((highByPeriod + lowByPeriod + 2 * openByPeriod) ,0)/4;
    R1 = rounddown((2 * PP - lowByPeriod)*4,0)/4;
    S1 = roundup((2 * PP - highByPeriod)*4,0)/4;
   

 r2 = if (hh > r1) then  roundup((PP + R1 - S1)*4,0)/4 else Double.NaN;
 r3 = if (hh > R2) then  roundup(( R2 + R1 - PP)*4,0)/4 else Double.NaN;

 s2 = if (ll < s1) then  roundup((pp+s1-r1)*4,0)/4 else Double.NaN;
 s3 = if (ll < s2) then  roundup((s2+s1-pp)*4,0)/4 else Double.NaN;
}

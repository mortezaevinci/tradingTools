...
script selectcross {
    ...
    plot crossauto = if (Momentumvalid) then (data > 0 and data < 8 and data != 4) else data;
...
def upw = crosspivots(crossoverlength, PilotsAggw, CrossingDirection.ABOVE, displace, BarsAggw)."upcrossd";
...
plot pupw =  upw;#selectcross(upw, Momentum > 0);
plot pupm =  upm;#selectcross(upm, Momentum > 0);

def updg=Momentum > 0 and fastmoveup and selectcross(upd, Momentum > 0);
def upwg=Momentum > 0 and fastmoveup and selectcross(upw, Momentum > 0);
...
def upcolorw = if (upwg) then 0 else 255;
def upcolorm = if (upmg) then 0 else 255;

pupd.AssignValueColor(CreateColor(upcolord, 255, upcolord));
pupw.AssignValueColor(CreateColor(upcolorw / 2, 255, upcolorw / 2));
...
plot pdnw = dnw;#selectcross(dnw, Momentum < 0);
plot pdnm = dnm;#selectcross(dnm, Momentum < 0);

def dndg=Momentum < 0 and fastmovedn and selectcross(dnd, Momentum < 0);
def dnwg=Momentum < 0 and fastmovedn and selectcross(dnw, Momentum < 0);

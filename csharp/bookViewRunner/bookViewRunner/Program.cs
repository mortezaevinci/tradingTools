...
namespace MHA
{
    class Program
    {
        ...
        public static void Main(string[] args)
        {
                ...
                Console.WriteLine("bookviewrunner.exe s=commaSeparatedSymbols f=filterlimit t=sleeplimit d=\"c:\\data\\directory\\\" h=\"17:30:00\"");
               
            string[] symbols =null;
            string filterlimit ="50";
            int overallSleepTime=60000;
            DateTime t_turnoff = DateTime.Parse("2020/12/12 23:59:00.000");
            ...
            foreach(string arg in args)
            {
                ...
                if (aa.Count()==2)
                {
                    if (aa[0].Equals("s")) symbols = aa[1].Split(new char[] { ',' });
                    if (aa[0].Equals("f")) filterlimit= aa[1];
                    if (aa[0].Equals("t")) overallSleepTime = Int32.Parse(aa[1]);
                    if (aa[0].Equals("d")) directory = String.Concat(aa[1].TrimEnd(new char[] { '"' }), "\\");
                    if (aa[0].Equals("h")) t_turnoff = DateTime.Parse(String.Format("2020/12/12 {0}.000",aa[1]));
                    Console.WriteLine(arg);
            ...
            {
                string dir = String.Concat(@directory, symbol, @"\");
            ...
            try
            {
                ...
                while (p.working)//sensecore.ImportProgress < 100)
                {
                 ...
                 //  ConsoleKeyInfo k=  Console.ReadKey(); // sticks to here
                    ...
                    if (dateTimeNY.TimeOfDay > t_turnoff.TimeOfDay)
                    {
                ...
                Console.WriteLine("Join and quit.");
                workerThreadImport.Join(5000);
                p.TearDown();
                p.CloseDriver();
        ...
        public long checkprogress(int overallSleepTime,string[] symbols, string directory)
        {
            ...
            */
            ...
            while (working)
            {
                        ...
                        if (directory != null)
                        {
                            ...
                            */
                            ...
                            for (int wi = 0; wi < 3; wi++)
                            {
                                try
                            {
                                using (Stream stream = File.Open(String.Concat(@directory, symbol , @"\", symbol, "_book_realtime.bn2"), FileMode.Create))
                                {
                            ...
                            for (int wi = 0; wi < 3; wi++)
                            {
                                try
                                {
                                    using (Stream stream = File.Open(String.Concat(@directory, symbol, @"\", symbol, "_book_history ", dtsdate, ".bn2"), FileMode.Append))
                                    {
                                        mdd.writeSimpleBinary(stream);
                                        writeok = true;
                                    }
                ...
                if (working) Thread.Sleep(mastersleep);
            }
        ...
        public void OpenUrl()
        {
            ...
            homeURL = "https://datastore.nasdaq.com/BookViewer.aspx";

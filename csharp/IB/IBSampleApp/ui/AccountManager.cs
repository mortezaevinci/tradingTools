...
namespace IBSampleApp.ui
{
    class AccountManager
    {
        ...
        public Dictionary<string, PositionMessage> positions = new Dictionary<string, PositionMessage>();
        ...
        public void HandlePosition(PositionMessage positionMessage)
        {
                 ...
                 */
                      ...
                      bool modified = false;
                      string key = ContractDefinition.Tools.uniqueKey(positionMessage.Contract);


                      if (positions.ContainsKey(key))
                      {
                          positions[key] = positionMessage.Copy();
                          positions[key].Position = positionMessage.Position;
                          modified = true;

                    //modify it on grid
                    int gcnt = positionsGrid.Rows.Count;
                          for (int i = 0; i < gcnt - 1; i++)
                          {
                              string cs = ContractDefinition.Tools.uniqueKey(positions[key].Contract);
                              if (cs.Equals((string)positionsGrid[0, i].Value))
                              {
                                  positionsGrid[0, i].Value = ContractDefinition.Tools.uniqueKey(positions.ElementAt(i).Value.Contract);
                                  positionsGrid[1, i].Value = positions[key].Account;
                                  positionsGrid[2, i].Value = positions[key].Position;
                                  positionsGrid[3, i].Value = positions[key].AverageCost;
                                  positionsGrid[4, i].Value = "FLAT";
                              }
                          }
                      }
                      ...
                      if (!modified)
                      {
                          try
                          {
                        //xxxmor, for now skip "STK" ones that have a quantity of 1
                        if (!positionMessage.Contract.SecType.Equals("STK") || positionMessage.Position >= 1 || positionMessage.Position == 0 || positionMessage.Position < -1 || showpos1)
                              {
                                  positions.Add(key, positionMessage.Copy());
                                  positionsGrid.Rows.Add(1);
                                  int i = positionsGrid.Rows.Count - 1;
                                  positionsGrid[0, positionsGrid.Rows.Count - 1].Value = ContractDefinition.Tools.uniqueKey(positions.ElementAt(i).Value.Contract);
                                  positionsGrid[1, positionsGrid.Rows.Count - 1].Value = positions.ElementAt(i).Value.Account;
                                  positionsGrid[2, positionsGrid.Rows.Count - 1].Value = positions.ElementAt(i).Value.Position;
                                  positionsGrid[3, positionsGrid.Rows.Count - 1].Value = positions.ElementAt(i).Value.AverageCost;
                       
                                 positionsGrid[4, positionsGrid.Rows.Count - 1].Value = "FLAT";
                              }
                          }
                          catch { }
                      }
                ...
                for (int i = 0; i < positions.Count; i++)
                {
                    ...
                    positionsGrid[0, positionsGrid.Rows.Count - 1].Value = Utils.ContractToString(positions.ElementAt(i).Value.Contract);
                    positionsGrid[1, positionsGrid.Rows.Count - 1].Value = positions.ElementAt(i).Value.Account;
                    positionsGrid[2, positionsGrid.Rows.Count - 1].Value = positions.ElementAt(i).Value.Position;
                    positionsGrid[3, positionsGrid.Rows.Count - 1].Value = positions.ElementAt(i).Value.AverageCost;
                    positionsGrid[4, positionsGrid.Rows.Count - 1].Value = "FLAT";

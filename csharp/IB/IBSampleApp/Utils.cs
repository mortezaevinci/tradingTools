...
namespace IBSampleApp.util
{
    class Utils
    {
        ...
        {
            //return contract.Symbol + " " + contract.SecType + " " + contract.Currency + " @ " + contract.Exchange + "|" + contract.LocalSymbol;
            if (contract.SecType.Equals("STK"))
                return contract.Symbol + " " + contract.Currency + " " + contract.Exchange + " " + contract.PrimaryExch;

            if (contract.SecType.Equals("OPT"))
            {
                if (String.IsNullOrEmpty(contract.Right)) return "ERROR";
                if (String.IsNullOrEmpty(contract.LastTradeDateOrContractMonth)) return "ERROR";
                string rr = contract.Right.Substring(0, 1);
                string shortdate = contract.LastTradeDateOrContractMonth.Substring(2, contract.LastTradeDateOrContractMonth.Length - 2);
                return String.Format("{0} {3}{2}{1}", contract.Symbol, contract.Strike, rr, shortdate);
            }
